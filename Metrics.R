#Metrics à coder une fois que la simulation sera prête et sans bugg
#visible si tout est monitoré correctement et que les graphs s'affichent sans soucis
#!!! des modifications de noms sont regulierement faites dans le code initial
#Reecrire ces metrics une à une de sorte à eviter les erreurs de call

# Performance opérationnelle

### Taux de production
# Calcul du taux de production
production_rate <- get_mon_performance(BIOTECHNIZ, type = "throughput")
...
service_rate <- get_mon_performance(BIOTECHNIZ, type = "service")
...
mean_resource_utilization <- get_mon_resources(BIOTECHNIZ, aggregate=TRUE)$data[, "utilization"]
...
availability_rate <- get_mon_resources(BIOTECHNIZ, aggregate = TRUE)$data[, "availability"]
...
rejection_rate <- get_mon_attributes(BIOTECHNIZ, "rejection_rate")
...
mean_in_production <- get_mon_resources(BIOTECHNIZ, aggregate = TRUE)$data[, "queue_length_Lot Fini"]
...
stock_rotation <- get_mon_attributes(BIOTECHNIZ, "stock_rotation")
...
mean_restock_time <- get_mon_attributes(BIOTECHNIZ, "mean_restock_time")
...
mean_order_processing_time <- get_mon_attributes(BIOTECHNIZ, "mean_order_processing_time")
...
mean_transit_time <- get_mon_attributes(BIOTECHNIZ, "mean_transit_time")
...
mean_waiting_time <- get_mon_attributes(BIOTECHNIZ, "mean_waiting_time")
...
customer_satisfaction_rate <- get_mon_attributes(BIOTECHNIZ, "customer_satisfaction_rate")
...
mean_delivery_time <- get_mon_attributes(BIOTECHNIZ, "mean_delivery_time")
...
order_accuracy_rate <- get_mon_attributes(BIOTECHNIZ, "order_accuracy_rate")
...
return_rate <- get_mon_attributes(BIOTECHNIZ, "return_rate")
